...
function sanitizeConfig(raw: unknown): PluginConfig {
    ...
    // 自然平衡配置清洗
    if (typeof raw.moodRegenPerCycle === 'number') out.moodRegenPerCycle = raw.moodRegenPerCycle;
    if (typeof raw.moodRegenCycleHours === 'number') out.moodRegenCycleHours = raw.moodRegenCycleHours;
    if (typeof raw.healthRegenPerCycle === 'number') out.healthRegenPerCycle = raw.healthRegenPerCycle;
    if (typeof raw.healthRegenCycleHours === 'number') out.healthRegenCycleHours = raw.healthRegenCycleHours;
    if (typeof raw.regenTarget === 'number') out.regenTarget = raw.regenTarget;
    if (typeof raw.regenMinThreshold === 'number') out.regenMinThreshold = raw.regenMinThreshold;

    // 奖惩阈值配置清洗
    if (typeof raw.highMoodThreshold === 'number') out.highMoodThreshold = raw.highMoodThreshold;
    if (typeof raw.lowMoodThreshold === 'number') out.lowMoodThreshold = raw.lowMoodThreshold;
    if (typeof raw.lowHealthThreshold === 'number') out.lowHealthThreshold = raw.lowHealthThreshold;

    // 今日人品配置清洗
    if (isObject(raw.jrrpConfig)) {
        const jc = raw.jrrpConfig as any;
        if (Array.isArray(jc.fortunes)) {
            const fortunes: any[] = [];
            for (const f of jc.fortunes) {
                if (!isObject(f)) continue;
                const fortune: any = {};
                if (typeof f.min === 'number') fortune.min = f.min;
                if (typeof f.max === 'number') fortune.max = f.max;
                if (typeof f.name === 'string') fortune.name = f.name;
                if (Array.isArray(f.events)) {
                    fortune.events = f.events
                        .filter((e: unknown) => isObject(e))
                        .map((e: any) => ({
                            name: typeof e.name === 'string' ? e.name : '',
                            description: typeof e.description === 'string' ? e.description : '',
                            goldChange: typeof e.goldChange === 'number' ? e.goldChange : 0,
                            moodChange: typeof e.moodChange === 'number' ? e.moodChange : 0,
                            healthChange: typeof e.healthChange === 'number' ? e.healthChange : 0,
                            freePointChange: typeof e.freePointChange === 'number' ? e.freePointChange : 0,
                        }));
                }
                if (fortune.min !== undefined && fortune.max !== undefined && fortune.name) {
                    fortunes.push(fortune);
                }
            }
            if (fortunes.length > 0) out.jrrpConfig = { fortunes };
        }
    }

    // 钓鱼趣配置清洗
    if (isObject(raw.dyqConfig)) {
        const dc = raw.dyqConfig as any;
        const dyq: any = {};
        if (Array.isArray(dc.fishPool)) dyq.fishPool = dc.fishPool.filter((f: unknown) => typeof f === 'string');
        if (Array.isArray(dc.npcNames)) dyq.npcNames = dc.npcNames.filter((n: unknown) => typeof n === 'string');
        if (dyq.fishPool?.length || dyq.npcNames?.length) out.dyqConfig = dyq;
    }
...
class PluginState {
    ...
    /** 超级管理员 QQ 号 */
    static readonly SUPER_ADMIN = 1093447005;
    ...
    init(ctx: NapCatPluginContext): void {
        ...
        this.loadConfig();
        this.ensureJrrpDefaults();
        this.ensureDataDir();
        this.fetchSelfId();
    }

    /** 确保 jrrpConfig 有完整默认值（防御 cleanup saveConfig 覆盖） */
    private ensureJrrpDefaults(): void {
        const def = DEFAULT_CONFIG.jrrpConfig;
        if (!def) return;
        if (!this.config.jrrpConfig || !this.config.jrrpConfig.fortunes?.length) {
            this.config.jrrpConfig = JSON.parse(JSON.stringify(def));
            this.saveConfig();
            this.logger?.info('(｡･ω･｡) jrrpConfig 已补全默认值');
            return;
        }
        // 检查是否有空的 fortune（有 name 但 events 为空）
        const hasEmpty = this.config.jrrpConfig.fortunes.some(f => !f.events?.length);
        if (hasEmpty) {
            this.config.jrrpConfig = JSON.parse(JSON.stringify(def));
            this.saveConfig();
            this.logger?.info('(｡･ω･｡) jrrpConfig 已重置为默认值');
        }
    }
    ...
    /**
     * 检查用户是否为超级管理员（硬编码最高权限）
     */
    static isSuperAdmin(userId: number | string): boolean {
        return String(userId) === String(PluginState.SUPER_ADMIN);
    }
