...
const work: Work = {
    ...
    settle: async (ctx: WorkContext) => {
        const attrValue = getAttributeValue(ctx.player, 'str');
        // 时薪 = 基础20 + 属性饱和加成(上限55)，无限属性时薪→75
        const hourlyPay = 20 + saturate(attrValue, 55, 4.5);
        const totalPay = Math.round(hourlyPay * (work.duration || 6) * (0.95 + Math.random() * 0.1));
